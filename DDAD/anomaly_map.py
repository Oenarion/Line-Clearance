...
def heat_map(output, target, FE, config, mask=None):
    '''
    ...
    i_d = pixel_distance(output, target, mask)
    f_d = feature_distance((output),  (target), FE, config, mask)
    f_d = torch.Tensor(f_d).to(config.model.device)
    # print(f"i_d shape: {i_d.shape}")
    # print(f"f_d shape: {f_d.shape}")
    # plt.imshow(np.transpose(i_d.detach().cpu().numpy()[0], (1,2,0)))
    # plt.title("pixel distance")
    # plt.show()
    # plt.imshow(np.transpose(f_d.detach().cpu().numpy()[0], (1,2,0)))
    # plt.title("feature distance")
    # plt.show()
    # if mask is not None:
    #     mask = mask.to(config.model.device)
    #     i_d = i_d * mask  # Zero out distances in invalid regions
    #     f_d = f_d * mask  # Zero out feature distances in invalid regions
...
def pixel_distance(output, target, mask):
    '''
    Pixel distance between image1 and image2
    '''
    distance_map = torch.mean(torch.abs(output - target), dim=1).unsqueeze(1)
    if mask is not None:
        mask = mask.to(distance_map.device)
        distance_map = distance_map * mask  # Apply mask
    return distance_map




def feature_distance(output, target, FE, config, mask):
    '''
    ...
    for i in range(len(inputs_features)):
        ...
        if mask is not None:
            mask = mask.to(a_map.device)
            a_map = a_map * mask  # Apply mask
        anomaly_map += a_map
    return anomaly_map 
